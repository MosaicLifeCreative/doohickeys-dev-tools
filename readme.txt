...
Upgrade to unlock advanced tools and power-user features:

* **Palette Generator** — Interactive color wheel with 7 harmony modes (complementary, analogous, triadic, and more). Export as CSS or SCSS variables.
...
* **Flexbox Generator** — Per-item controls (flex-grow, flex-shrink, flex-basis, align-self, order)
* **CSS Grid Generator** — Templates (Holy Grail, Dashboard, 12-Col, etc.), custom definitions, per-cell spans
* **Placeholder Image** — Social media presets and Data URI export
* **Schema.org Generator** — 5 additional types (Product, Person, Organization, Event, Recipe)
* **QR Code Generator** — Custom logo overlay
...
1. Upload the `mlc-web-dev-tools` folder to `/wp-content/plugins/`
...
No. Web Dev Tools only loads on its own admin page. It adds zero scripts, styles, or markup to your public-facing site.
...
Pro unlocks 5 full tools (Palette Generator, Code Formatter, Diff Checker, HTML Table Generator, Test Data Generator) plus advanced features in 5 more tools (per-item Flexbox controls, Grid templates, social media presets, additional Schema.org types, and QR code logo overlay).
